...
function isContainNumber(num, digit) {
  let numFloor = Math.abs(Math.floor(num));
  const digitFloor = Math.abs(Math.floor(digit));

  if (numFloor === digitFloor) {
    return true;
  }

  while (numFloor > 0) {
    const currentDigit = numFloor % 10;

    if (currentDigit === digitFloor) {
      return true;
    }
